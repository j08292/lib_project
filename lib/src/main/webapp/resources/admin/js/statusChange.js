...
function bookReturn_event(rent_num,rent_status,list_title){
	if(rent_status == 0){//대출중 도서 -> 반납 처리
...
function bookRent_event(rent_num,list_title,rent_regdate){//대출대기 도서 -> 대출 처리
		location.href='updateRent.do?rent_num='+ rent_num+'&rent_status=0'+'&rent_regdate='+rent_regdate;
		alert("["+list_title+"] 이/가 대출 처리되었습니다.");
}
