...
$(document).ready(function(){
				...
				if(count < 0 || list == null){
					...
					$(list).each(function(index,item){
						...
						if($('#mem_id').val() && $('#mem_id').val()==item.mem_id){
							output += ' <input type="button" data-number="'+item.list_num+'" data-num="'+item.grade_num+'" data-id="'+item.mem_id+'" class="delete_button" value="삭제">';
						}/*else{
	...
	$('#grade_form').submit(function(event){
		...
		$.ajax({
			...
			success:function(data){
				...
				}else if(data.result=='alreadyWrite'){
	...
	$(document).on('click', '.delete_button', function(){
		...
		//글번호
		var list_num = $(this).attr('data-number');

		$.ajax({
			type:'post',
			data:{grade_num:grade_num, mem_id:mem_id,list_num:list_num},
