$(document).ready(function(){
				...
				if(count < 0 || list == null){
					...
					$(list).each(function(index,item){
						...
						if($('#userId').val() && $('#userId').val()==item.mem_id){
							output += '  <input type="button" data-num="'+item.review_re_num+'" data-id="'+item.mem_id+'" class="modify_button btn btn-default btn-sm" value="수정">';
							output += '<input type="button" data-num="'+item.review_re_num+'" data-id="'+item.mem_id+'" class="delete_button btn btn-default btn-sm" value="삭제">';
						}
	...
	$('#re_form').submit(function(event){
		if($('#review_re_content').val()==''){
			alert('내용을 입력하세요');
			$('#review_re_content').focus();
			return false;
		}
	...
	$('#review_re_content, #mreview_re_content').keyup(function(){
		// 남은 글자수 구하기.
		var inputLength = $(this).val().length;
		
		if(inputLength > 300){
			$(this).val($(this).val().substring(0,300));
			if($(this).attr('id')=='review_re_content'){
				//등록폼 글자수
	...
	$(document).on('click', '.modify_button', function(){
		...
		// 수정폼에 데이터 셋팅
		$('#mreview_re_num').val(num);
		$('#muserId').val(id);
		$('#mreview_re_content').val($('#i'+num+' p').text());
		
		// 입력한 글자수 셋팅
		var inputLength = $('#mreview_re_content').val().length;
		var remain = 300-inputLength;
		remain += '/300';
	...
	function initModifyForm(){
		...
		$('#mreview_re_num').val('');
		$('#muserId').val('');
		$('#mreview_re_content').val('');
		$('#mre_first .letter-count').text('300/300');
	}
	
	// 댓글 수정
	$('#mre_form').submit(function(event){
		if($('#mreview_re_content').val()==''){
			alert('내용을 입력하세요');
			$('#mreview_re_content').focus();
			return false;
		}
		...
		$.ajax({
			...
			success:function(data){
				if(data.result=='logout'){
					...
					$('#i'+$('#mreview_re_num').val() + ' p').html($('#mreview_re_content').val());
					//수정폼 초기화
					initModifyForm();
				}else{
					alert('수정시 오류 발생');
