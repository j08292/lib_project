				...
				if(count < 0 || list == null){
					...
					$(list).each(function(index,item){
						...
						if($('#userId').val() && $('#userId').val()==item.mem_id){
							output += '<input type="button" data-num="'+item.review_re_num+'" data-id="'+item.mem_id+'" class="modify_button" value="수정">';
							output += '<input type="button" data-num="'+item.review_re_num+'" data-id="'+item.mem_id+'" class="delete_button" value="삭제">';
						}else{
