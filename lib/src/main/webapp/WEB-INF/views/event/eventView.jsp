	...
	<div class="container clearfix">
		...
		<div class="col_full" align="center">
			<c:if test="${!empty event.event_filename}">
				<img alt="이벤트사진"
					src="${pageContext.request.contextPath}/event_upload/${event.event_filename}">
			</c:if>
		</div>

		<hr size="1" width="100%">

		<div class="col_full">${event.event_content}</div>
		<hr size="1" width="100%">

		<!-- 관리자만 할 수 있도록 레벨값을 줘야함 -->
		<p align="right">
			<input type="button" value="수정"
				onclick="location.href='update.do?event_num=${event.event_num}'">
			<input type="button" value="삭제"
				onclick="location.href='delete.do?event_num=${event.event_num}'">
		</p>
		...
		<div class="panel-group">
			...
			<form id="re_form">
				<input type="hidden" name="event_num" value="${event.event_num}"
					id="event_num"> <input type="hidden" name="mem_id"
					value="${userId}" id="userId">
				<textarea rows="4" name="event_re_content" maxlength="1000"
					id="event_re_content" class="col_full"
					<c:if test="${empty userId}">disabled="disabled"</c:if>><c:if
						test="${empty userId}">로그인해야 작성할수 있습니다.</c:if></textarea>
			...
			<div id="modify_div" style="display: none;">
				<form id="mre_form">
					<input type="hidden" name="event_re_num" id="mre_no"> <input
						type="hidden" name="mem_id" id="muserId">
